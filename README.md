...
| Platform owner | Separate AS Communications control plane for firm/company provisioning, scoped administrator creation and editing, password reset, suspension and employee capacity limits. Company creation can also provision a company admin and HR admin. Passwords are hashed and never retrievable. Company limits reserve slots from the firm limit, so their sum cannot exceed it. Enabled only when `PLATFORM_OWNER_PASSWORD` is configured. |
